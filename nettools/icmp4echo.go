...
func (p *pkg) Icmp4Echo(ctx context.Context, target netip.Addr, opts ...Icmp4EchoOption) ([]Icmp4EchoResponse, error) {
	...
	for i := 0; i < opt.Count; i++ {
		r, err := rawPing4(ctx, target, opt.TTL, opt.ListenAddress, opt.ReadTimeout, opt.IcmpID, opt.IcmpSeq, opt.AllowAllErrors)
...
func rawPing4(ctx context.Context, target netip.Addr, ttl int, listenAddress netip.Addr, readTimeout time.Duration, icmpID int, icmpSeq int, allowAllErrors bool) (response Icmp4EchoResponse, err error) {
	...
	// log.Info("resp", "peer", peer, "error", err)
	endtime := time.Now()
