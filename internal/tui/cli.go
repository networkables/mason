...
func newTuiCLI(m MasonReaderWriter, r *lipgloss.Renderer) *tuicli {
	ti := textinput.New()
...
func (c tuicli) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case tea.KeyEnter:
			...
			c.cmds, cmd1 = c.processCmd(ctx, cmdstr)
			c.input.SetValue("")
		}
...
func (c tuicli) processCmd(ctx context.Context, s string) ([]string, tea.Cmd) {
	log.Debug("cmd: " + s)
	var cmd tea.Cmd
	cmdstr := s
	...
	case cmdAdd:
		if len(args) > 1 {
			...
			case cmdAddNetwork:
				if len(args) > 2 {
					prefix := args[2]
					newnet, err := model.New(prefix, prefix)
					if err != nil {
						s += "\n" + "error creating network: " + err.Error()
					} else {
						err := c.mason.AddNetwork(ctx, newnet)
						if err != nil {
							s += "\n" + "error creating network: " + err.Error()
						} else {
							s += "\n" + "network added"
